...
def selectsample(model, x_test, y_test, delta, iterate):
    ...
    for i in range(iterate):
        ...
        print("select: failures: {!s} | total: {!s}".format(np.sum(pred != label),orig_sample.shape[0]))
        ...
        print("random: failures: {!s} | total: {!s}".format(np.sum(pred != label),orig_sample.shape[0]))
...
if __name__ == '__main__':
    ...
    Xval, x_test, yval, y_test = train_test_split(Xtemp, ytemp, train_size=val, random_state=0)

    Xtrain = Xtrain.reshape(train,width,height,channels)
    Xval = Xval.reshape(val,width,height,channels)
    x_test = x_test.reshape(test,width,height,channels)

    model = models.load_model("../models/modelB.h5")
