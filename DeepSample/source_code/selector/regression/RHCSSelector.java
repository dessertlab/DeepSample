...
public class RHCSSelector extends TestCaseSelector_reg {
	...
	public void selectAndRunTestCase(double _desiredPercentageAccuracy) {
		...
		while (indexCurrentTest < totalTests){
			...
			/*****ELIMINATO IMPORTANCE SAMPLING: RIFERIRSI A "Experiment" DI ICSE2017 WORKSPACE PER RECUPERARE ULTIMA VERSIONE *******/

			// IN AN INTERATIVE VERSION, COMPUTE numberOfTestCurrentIteration
			//numberOfTestCurrentIteration = computeNumberOfTests();
...
//		System.out.println("\n ************ Reliability Estimate: "+(estimatedOutput.doubleValue()));
...
//		System.out.println("\n ************ Total number of executed test cases: "+indexCurrentTest);
		...
		this.estimate = (estimatedOutput.doubleValue());
	...
	private void selectAndExecute_RHC(ArrayList<TestCase> population, int sampleSize, MutableDouble sampleOutput, MutableDouble estimatedOutput, MutableDouble estimatedVariance, MutableDouble estiamtedCI_HalfWidth, ArrayList<TestCase> FailedTestCases) throws Exception {
		...
		/**********************	STEP 2: create g homogeneous groups **********************/
		...
		for(int i=0;i<numberOfGroups; i++){	
			...
			unreliability[i] = testCaseToExecute.runTestCase_squared_error("running...");
		...
		estimatedOutput.setValue(Y/originalSize);
